...
class AlternativeJobMonitor:
    ...
    def send_telegram_notification(self, new_jobs):
        ...
        try:
            ...
            for i, job in enumerate(new_jobs[:5], 1):  # Limit to 5 jobs per message
                ...
                message += f"🌐 Source: {job.get('source', 'Unknown')}\n"
                message += f"📝 {job['snippet'][:120]}...\n"
    ...
    def run_monitoring(self):
        ...
        if all_new_jobs:
            ...
            elif not self.telegram_config:
                # Show jobs in console if no notifications configured
                self.print_jobs_summary(all_new_jobs)
...
def main():
    ...
    if not has_email and not has_telegram:
        print("⚠️  No notification configuration found!")
        print("Jobs will be displayed in console only.")
    ...
    monitor = AlternativeJobMonitor(email_config, telegram_config if has_telegram else None)
